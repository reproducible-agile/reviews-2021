...
2. Build the image with the database (file paths depends on working directory)
   ...
   `docker build --no-cache -t agile-reproduction-2020-009 --file docker-postgres/Dockerfile .`

3. Run the image without the mount (file paths depends on working directory)

   `docker run --name docker-postgres --rm -it -p '5432:5432' agile-reproduction-2020-009`
