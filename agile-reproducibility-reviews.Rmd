...
if(params$private_info) {
  review_data %>%
    #TODO re-add when accepted papers are decided !!!! dplyr::filter(decision == "ACCEPT" | decision == "accept?") %>%
    filter(type == "Full-paper submission") %>%
    arrange(id) %>%
    kable() %>%
    kable_styling("striped") %>%
...
The review comments for each paper are stored in an HTML file next to the PDF of the submission and uploaded to [a private share](https://drive.google.com/drive/folders/1A_fqbv1I5hdwdNf3Kr8adB13cRqmDcqH) so that reproducibility reviewers can access them.
The following includes the full reviews, including the names of the original reviewers, and cannot be publicly shared.

```{r upload_settings}
review_data_csv_file <- file.path(review_files_path, "review_data.csv")
```

1. Create HTML from review page and safe to `r review_files_path`
2. Write paper metadata (ID, decision, title) to a CSV file `r review_data_csv_file`
3. **Manually** [import the paper metadata into the spreadsheet](https://www.tillerhq.com/how-to-import-csv-into-a-google-spreadsheet/) (Select cell `A1` then "File" > "Import" > "Import File" > "My Drive" then search for `review_data` and find `review_data.csv` then select the file > "Replace data at selected cell" and click "Import data")

```{r accepted_fp_files_download_easychair, echo=FALSE, eval=TRUE}
if (params$private_info) {
  review_files <- review_data %>%
    filter(type == "Full-paper submission") %>%
    #TODO re-add when accepted papers are decided !!!! dplyr::filter(decision != "REJECT") %>%
...
4. Conduct your reproducibility review and write the report
    ...
    - If code is available on GitHub/Lab, you can fork the project into the [Reproducible AGILE organisation](https://github.com/reproducible-agile/) respectively the [GitLab subgroup "reviews"](https://gitlab.com/reproducible-agile/reviews); ask Daniel to get the permissions
...
6. Add a new component to the [OSF project for 2021 reproducibility reviews](https://osf.io/h64sd/)
...
congratulations to the acceptance of your submission "TITLE" as a full paper at the AGILE conference 2021. As part of the Reproducible AGILE initative (https://reproducible-agile.github.io/) I attempted to reproduce the results from your paper. Attached to this email you find my report on your results.
...
drive_put(media = "agile-reproducibility-reviews.html",
          name = paste0("agile-reproducibility-reviews_",
                 ...
                 ".html"),
          path = "https://drive.google.com/drive/folders/1Y2905cRV1APIE0fSbX7Kmy-xvRMXj_0y")
drive_put("agile-reproducibility-reviews.Rmd", path = "https://drive.google.com/drive/folders/1Y2905cRV1APIE0fSbX7Kmy-xvRMXj_0y")
```
